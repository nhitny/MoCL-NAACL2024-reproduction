...
class ContinualTrainerMTL(nn.Module):
    def __init__(
        ...
        self.model = model  # model đã được dispatch bởi accelerate, không cần .to() Nhitny
        # self.query_encoder = query_encoder.to(self.args.device)
        self.query_encoder = query_encoder  # tránh chuyển sang GPU nếu GPU đã đầy #Nhitny
        self.logger = logger
        ...
        try:
            ...
            self.learning_rate_list = [
                self.args.learning_rate for _ in range(self.num_tasks)
            ]
        ...
        self.logger.info(
            f"***********************************lr: {self.learning_rate_list}***********************************"
        )
        self.logger.info(
            f"***********************************lr_scheduler_type: {self.args.lr_scheduler_type}***********************************"
        )
    ...
    def _prepare_optimizer(self, task_id=None):
        ...
        learning_rate = (
            self.learning_rate_list[task_id]
            if self.learning_rate_list is not None
    ...
    def train(
            ...
            for task_id, task in enumerate(self.task_list):
                if task_id == 0 or self.model.config.multi_peft_modules:
                    self.model = get_peft_model(
                        self.model, self.peft_config, adapter_name=task
                    )
                ...
                num_examples_dataset = (
                    train_batch[task_id] * self.args.per_device_train_batch_size
                )
                num_examples_training = (
                    num_training_steps * self.args.per_device_train_batch_size
                )
                ...
                num_training_steps_all_epochs = math.ceil(
                    self.num_train_epochs * num_update_steps_per_epoch
                )
                ...
                # Train!
                self.logger.info(
                    f"***** Running training - task {task_id}: {task}*****"
                )
                ...
                self.logger.info(
                    f"  Total optimization steps = {num_training_steps_all_epochs}"
                )
                ...
                for epoch in range(self.num_train_epochs):
                    ...
                    for batch_index in tqdm(
                        range(num_train_batch), total=num_training_steps_all_epochs
                    ):
                        train_input, train_gt = self._process_data(
                            train_loader[task], mode="train"
                        )
                        query_embed = self._get_query_embed(
                            train_input,
                            task,
                            do_statistics=(
                                epoch == 0 and self.model.config.task_identify_epi
                            ),
                        )
                        train_loss, train_pred = self.compute_loss(
                            self.model,
                            train_input,
                            task,
                            mode="train",
                    ...
                    eval_res_ = self.eval(
                        val_loader[task], val_batch[task_id], task, mode="val"
                    )
                    ...
                    es_task_name = (
                        ...
                        else self.task_list[0]
                    )
                    if self.early_stopping_patience > 0 and (
                        self.early_stopping(
                            eval_res_[self.metric], self.model, es_task_name
                        )
                        or epoch == self.num_train_epochs - 1
                    ):
                        best_metric_eval = self.early_stopping.best_score
                        best_epoch = eval_res[self.metric].index(best_metric_eval)

                        self.logger.info(
                            f"Early stop!!! Best epoch for [{task}]: {best_epoch}"
                        )
                        self.logger.info(
                            f"Best evaluation {self.metric} [{task}]: {round(best_metric_eval, 4)}"
                        )

                        # load weights of the best model
                        model_path = os.path.join(
                            self.early_stopping.save_path, f"best_checkpoint_{task}.pth"
                        )
                        self.logger.info(
                            f"Loading weights from the best model (best epoch {best_epoch})..."
                        )
                        set_peft_model_state_dict(
                            self.model,
                            self.early_stopping.best_model,
                            adapter_name=es_task_name,
                        )
                        if task_id != len(self.task_list) - 1 and os.path.exists(
                            model_path
                        ):
                            os.remove(model_path)

                        best_res_test = self.eval(
                            test_loader[task], test_batch[task_id], task, mode="test"
                        )

                        self.logger.info(
                            f"Best test {self.metric} [{task}]: {round(best_res_test[self.metric], 4)}"
                        )
                ...
                if (
                    self.model.config.multi_peft_modules
                    and not self.model.config.disentangle_modules
                ):
        ...
        if self.model.config.disentangle_modules:
            ...
            self.logger.info(
                f"------- Forward pass evaluation (seed-{self.seed}) -------"
            )
            for k in fwd_pass_results_metric.keys():
                self._log_final_results(
                    test_loader, fwd_pass_results_metric[k], key=f"{k}"
                )

            test_results_metric = defaultdict(list)
            for t_id, t in enumerate(test_loader.keys()):
                self.logger.info(
                    f"------- Final evaluation (seed-{self.seed}) on task [{t}] -------"
                )
                test_res = self.eval(
                    test_loader[t], test_batch[t_id], t, mode="test", final=True
                )
        ...
        for k in test_results_metric.keys():
            self._log_final_results(
                test_loader, test_results_metric[k], fwd_pass_results_metric[k], key=k
            )
        ...
        if self.model.config.task_identify_epi:
            ...
            for t_id, t in enumerate(test_loader.keys()):
                ...
                self.logger.info(
                    f"------- Final EPI evaluation (seed-{self.seed}) on task [{t}] -------"
                )
                test_res = self.epi_eval(
                    test_loader[t], test_batch[t_id], t_id, mode="test"
                )
                for k, v in test_res.items():
            ...
            for k in epi_test_results_metric.keys():
                self._log_final_results(
                    test_loader, epi_test_results_metric[k], key=f"EPI-{k}"
                )
    ...
    def _get_query_embed(self, model_inputs, task=None, do_statistics=False):
        device = next(self.query_encoder.parameters()).device #Nhitny
        input_ids = model_inputs["input_ids"].to(device) #Nhitny
        attention_mask = model_inputs["attention_mask"].to(device) #Nhitny
    ...
    def eval(self, loader, batch, task, mode="test", final=False):
        ...
        with torch.no_grad():
            for batch_index in tqdm(range(batch)):
                ...
                generated_tokens = self.compute_loss(
                    self.model,
                    input,
                    task,
                    mode=mode,
        ...
        if final:
            ...
            self.logger.info(
                f"***** Task {task} - Epoch {self.model.epoch}: {mode} (seed {self.seed}) *****"
            )
    ...
    def epi_eval(self, loader, batch, task_id, mode="test"):
        ...
        self.logger.info(
            f"***** Task {self.task_list[task_id]} - Final EPI evaluation: {mode} *****"
        )
    ...
    def _postprocess_generated_tokens(self, generated_tokens):
        generated_tokens = np.where(
            generated_tokens == -100, self.tokenizer.pad_token_id, generated_tokens
        )
    ...
    def _log_final_results(
        self, test_loader, test_results, fwd_pass_results=None, key="acc"
    ):
        self.logger.info(
            f"***********************************seed: {self.seed} - {key}***********************************"
        )
        ...
        if fwd_pass_results:
            ...
            self.logger.info(
                f"Average test_tasks_{key} (fwd_pass): {round(sum(fwd_pass_results)/len(fwd_pass_results), 4)}"
            )

        self.logger.info(f"test_tasks_{key}: {test_results}")
        self.logger.info(
            f"Average test_tasks_{key}: {round(sum(test_results)/len(test_results), 4)}"
        )
    ...
    def _initialize_epi_variables(self):
        ...
        self.accumulate_shared_covs = nn.Parameter(
            torch.zeros(self.query_size, self.query_size), requires_grad=False
        )
        self.cov_inv = nn.Parameter(
            torch.ones(self.query_size, self.query_size), requires_grad=False
        )
    ...
    def _statistics_task_mean_cov(self, task, task_id):
        ...
        shared_cov = torch.stack(cov_over_classes).mean(
            dim=0
        )  # dim x dim (averaged over N_class)

        self.task_means_over_classes.append(
            nn.Parameter(mean_over_classes.cuda(), requires_grad=False)
        )
        ...
        self.cov_inv = nn.Parameter(
            torch.linalg.pinv(
                self.accumulate_shared_covs / (task_id + 1), hermitian=True
            ),
            requires_grad=False,
        )  # E: Computes the pseudoinverse (Moore-Penrose inverse)
    ...
    def _print_task_infer_acc(self):
        ...
        for task_id, task in enumerate(self.task_list):
            avg_acc = round(
                sum(self.task_infer_acc[task_id]) / len(self.task_infer_acc[task_id]), 4
            )
            task_infer_acc_all.append(avg_acc)
        self.logger.info(
            f"***********************************EPI task inference accuracy***********************************"
        )
    ...
    def _save_weight_dict(self, weight_matrix=None, valid_only=False):
        ...
        weight_list = (
            ...
            else self.model.key_encoder.attn_weights
        )
        ...
        for i, task in enumerate(self.task_list):
            weight_dict[task] = {
                self.task_list[j]: tensor_to_value(weight_list[i][j])
    ...
    def clean_gpu_memory(self):
        ...
        print(
            f"GPU {device_id}: Before clearing cache: Total Memory: {total_memory:.0f} GB, Allocated: {allocated_memory:.0f} GB, Cached: {cached_memory:.0f} GB"
        )
        ...
        print(
            f"GPU {device_id}: After clearing cache: Total Memory: {total_memory:.0f} GB, Allocated: {allocated_memory:.0f} GB, Cached: {cached_memory:.0f} GB"
        )
