...
def DataLoaderMTL(
    ...
    dataset_dir = "/workspace/nhitny/mcs/MoCL-NAACL-2024/datasets/mtl15"
    task_labels = {}
    ...
    def preprocess_function(examples):
        ...
        limit_input_len = (
            max_seq_length + max_target_length if mode == "train" else max_seq_length
        )
            ...
            if mode == "train":
                ...
                if len(tokenized_input) <= limit_input_len:
                    ...
                    input_wo_label = tokenizer.decode(
                        tokenized_input[:limit_input_len], skip_special_tokens=False
                    )
        ...
        model_inputs["label"] = model_inputs["input_ids"].masked_fill(
            ~label_mask, label_pad_token_id
        )
    ...
    def get_dataset_target_ids(dataset: list, task):
        for i, sample in enumerate(dataset):
            if (
                "label" not in sample
                or sample["label"] not in task_labels2id[task].keys()
            ):
                dataset.pop(i)
                i -= 1
            else:
    ...
    def get_dataloader(dataset, mode):
        ...
        sampler = (
            RandomSampler(dataset) if mode in ["train"] else SequentialSampler(dataset)
        )
        drop_last = (
            training_args.dataloader_drop_last if mode in ["train", "dev"] else False
        )
        ...
        return DataLoader(
            ...
            num_workers=0,  # Nhitny: set to 0 to avoid issues with GPU memory
    ...
    n_train_per_class = (
        16 if data_args.n_train_per_class is None else data_args.n_train_per_class
    )
    n_val_per_class = (
        50 if data_args.n_val_per_class is None else data_args.n_val_per_class
    )
    ...
    for ti, task in enumerate(task_list):
        ...
        for mode in mode_list:
            ...
            if mode == "train":
                train_dataset = select_subset_dataset(
                    dataset, task, n_train_per_class, training_args.seed
                )
                dataloader[task]["train"] = get_dataloader(train_dataset, mode="train")

                valid_seq_len[task] = get_valid_seq_len(
                    dataloader[task]["train"].dataset, tokenizer.pad_token_id
                )

            elif mode == "dev":
                dev_dataset = select_subset_dataset(
                    dataset, task, n_val_per_class, training_args.seed
                )
                ...
                if (
                    data_args.n_val_per_class and data_args.n_val_per_class <= 16
                ):  # debug mode
