...
def DataLoaderMTL(
    ...
    for ti, task in enumerate(task_list):
        ...
        offset[task] = (
            len(label_list[ti - 1]) + offset[task_list[ti - 1]] if ti > 0 else 0
        )
    ...
    def preprocess_function(examples):
        if task == "yahoo":
            ...
            for title, content, answer in zip(
                examples["title"], examples["content"], examples["answer"]
            ):
    ...
    def select_subset_dataset_per_label(
        dataset, task, n_train_per_class, n_val_per_class, seed
    ):
        np.random.seed(seed)
    ...
    def select_subset_dataset(dataset, task, n_train_total, n_val_per_class, seed):
        ...
        for l in range(n_labels):
            ...
            if n_train_per_class < 0:
                ...
                if l == n_labels - 1:
                    train_idx.extend(
                        train_pool[: n_train_total - n_train_per_class * (n_labels - 1)]
                    )
                else:
                    train_idx.extend(train_pool[:n_train_per_class])
            val_idx.extend(idx_total[-n_val_per_class:])
    ...
    for ti, task in enumerate(task_list):
        ...
        for mode in mode_list:
            ...
            dataset_path = os.path.join(
                "/workspace/nhitny/mcs/MoCL-NAACL-2024/datasets/mtl5",
                f"{mode}/{task}.csv",
            ).replace(
                "dev", "train"
            )  # Nhitny
            ...
            if mode == "train":
                ...
                n_train_per_class = (
                    2000
                    if data_args.n_train_per_class is None
                    else data_args.n_train_per_class
                )
                n_val_per_class = (
                    500
                    if data_args.n_val_per_class is None
                    else data_args.n_val_per_class
                )
                if data_args.n_train_total is not None:
                    train_idx, val_idx = select_subset_dataset(
                        dataset,
                        task,
                        data_args.n_train_total,
                        n_val_per_class,
                        training_args.seed,
                    )
                else:
                    train_idx, val_idx = select_subset_dataset_per_label(
                        dataset,
                        task,
                        n_train_per_class,
                        n_val_per_class,
                ...
                if logger is not None:
                    logger.info(
                        f"Task [{task}]-train first 2 examples: \n {train_dataset[:2]['content'].values}"
                    )
                ...
                if (
                    data_args.n_val_per_class and data_args.n_val_per_class < 50
                ):  # debug mode
                    dataset = dataset.iloc[: data_args.n_val_per_class]
                if data_args.max_predict_samples is not None:
                    dataset = dataset.sample(frac=1).iloc[
