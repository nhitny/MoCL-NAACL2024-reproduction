...
#             if type(label_list[0]) == list:
#                 result["label"] = [
#                     (label2id[task][l] if l != -1 else -1) for l in examples["label"]
#                 ]
#             else:
#                 result["label"] = [
...
#     #     )
#     #     dataloader[task][mode] = DataLoader(
#     #         dataset,
...
#                 data_args.dataset_path, f"{mode}/{task}_{mode}.tsv"
#             )
#             if not os.path.exists(dataset_path):
#                 raise FileNotFoundError(f"❌ Không tìm thấy file: {dataset_path}")
#             dataset = pd.read_csv(dataset_path, sep="\t").dropna()

#             # ✅ Gán task_id để truyền vào model
...
def DataLoaderMTL(
    ...
    def preprocess_function(examples):
        ...
        if label2id is not None and "label" in examples:
            if type(label_list[0]) == list:
                result["label"] = [
                    (label2id[examples["task_id"][i]][l] if l != -1 else -1)
        ...
        if "task_id" in examples:
            result["task_id"] = examples["task_id"]
        #Nhitny
        return result
    ...
    for task in task_list:
        ...
        for mode in mode_list:
            ...
            if not os.path.exists(dataset_path):
                raise FileNotFoundError(f"❌ Không tìm thấy file: {dataset_path}")
            dataset = pd.read_csv(dataset_path, sep="\t").dropna()
            dataset["task_id"] = task  # ✅ Gán task_id vào dataset

            if mode == "train" and data_args.max_train_samples is not None:
                dataset = dataset.sample(
                    n=min(len(dataset), data_args.max_train_samples)
                )
            elif mode == "dev" and data_args.max_eval_samples is not None:
                dataset = dataset.sample(
                    n=min(len(dataset), data_args.max_eval_samples)
                )
