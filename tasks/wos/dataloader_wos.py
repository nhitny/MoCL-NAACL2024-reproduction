...
def DataLoaderMTL(
    ...
    dataset_path = os.path.join(
        "/workspace/nhitny/mcs/MoCL-NAACL-2024/datasets/wos/WOS11967/", "wos.csv"
    )  # Nhitny
    dataset_all = pd.read_csv(
        dataset_path,
        header=None,
        names=["input_data", "label", "label_level_1", "label_level_2"],
    )
    ...
    for ti, task in enumerate(task_list):
        ...
        task_idx = np.where(
            dataset_all["label_level_1"].astype(str) == str(task)
        )  # Nhitny: convert to str for compatibility
        dataset[task] = dataset_all.iloc[task_idx]
        num_examples = len(dataset[task])

        n_train, n_val, n_test = (
            int(num_examples * 0.6),
            int(num_examples * 0.2),
            int(num_examples * 0.2),
        )
        ...
        if logger is not None:
            ...
            logger.info(
                f"Task-[{task}]-test first 2 examples: \n {test_dataset[:2]['input_data'].values}"
            )
