...
class TaskKeyEncoder(nn.Module):
    ...
    #Nhitny
    def _query_key_match(self, Q, K):
        device = Q.device  # đảm bảo mọi thứ nằm trên cùng thiết bị

        n_k = nn.functional.normalize(K, dim=-1).to(device)
        n_q = nn.functional.normalize(Q, dim=-1).detach().to(device)

        assert n_k.dim() == n_q.dim() == 2
        cos_sim = torch.einsum('bj,kj->bk', n_q, n_k)  # (batch_size, num_keys)

        # Ensure self.keys_mask is on the correct device and shape
        # keys_mask = self.keys_mask.to(device)  # giả sử: shape = (num_keys,)
        keys_mask = torch.tensor(self.keys_mask, dtype=torch.bool).to(device) #Nhitny
        
        # Đảm bảo clone trước khi sửa cos_sim (nếu cần)
        if keys_mask.any():
            cos_sim = cos_sim.clone()
            cos_sim[:, keys_mask] = -1e8
