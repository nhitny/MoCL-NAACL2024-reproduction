...
class RobertaPrefixContinualForSequenceClassification(RobertaPreTrainedModel):
        ...
        if config.task_specific_classifier:
            if type(config.label_list[0]) == list:
                self.classifiers = torch.nn.ModuleList(
                    [
                        torch.nn.Linear(config.hidden_size, len(labels))
    ...
    def get_prompt(
        ...
        outputs = self.prefix_encoder(
            input0, task_id, train, final, prompt_select_mode, task_id_list, id_pred_on
        )

        return (
            outputs if self.config.compose_prompts else (outputs, 0.0)
        )  # [ComposePrompts] / [vanilla CL & ProgressivePrompt & EPI without composition]
    ...
    def forward(
        ...
        **kwargs,  # Nhitny
        ...
        #Nhitny
        # ✅ Bắt đầu xử lý task_id từ kwargs hoặc từ đối số
        task_id = kwargs.get("task_id", task_id)  # lấy từ kwargs nếu có
        # task_id = kwargs.get("task_id", None)  # Nhitny lấy task_id từ kwargs
        print(f"[DEBUG] task_id nhận được: {task_id} | type: {type(task_id)}")


        # ✅ Nếu task_id là list hoặc tensor (batch), lấy phần tử đầu
        if isinstance(task_id, (list, torch.Tensor)):
            task_id = task_id[0]

        # ✅ Đảm bảo task_id là int hoặc str (tùy mô hình dùng dict hay list)
        try:
            task_id = int(task_id)
        except:
            task_id = str(task_id)

        # ✅ Debug kiểm tra task_id
        print(f"[DEBUG] task_id trong forward: {task_id}")
        ...
        if self.config.compose_prompts or self.config.task_identify_epi:
            ...
            x_query = self.process_query(
                x_embed, attention_mask, train, task_id, labels
            )

            if final:
                if (
                    id_pred_on or classifier_select_mode != "task_id"
                ):  # pred task ids for prompt selection and/or classifier selection
                    if not (
                        self.config.task_identify_epi
                        or self.config.classifier_match_embed
                        or self.config.classifier_match_key
                    ):
                        self.config.classifier_match_key = True
                    task_id_list, task_id_prob = self._get_pred_ids(
                        x_query, task_id, final
                    )

        past_key_values, match_loss = self.get_prompt(
            batch_size,
            x_query,
            task_id,
            train=train,
            ...
            id_pred_on=id_pred_on,
        )

        prefix_attention_mask = torch.ones(batch_size, past_key_values[0].shape[-2]).to(
            self.roberta.device
        )
        ...
        print(f"[CHECK] task_id type: {type(task_id)}, value: {task_id}") #Nhitny
        if self.config.task_specific_classifier:
            ...
            if train:
                ...
                if classifier_select_mode == "task_id":
                    ...
                    logits = (
                        torch.ones(batch_size, self.config.num_labels).to(
                            x_query.device
                        )
                    ) * (-float("inf"))
                    ...
                    for t_id in range(self.num_tasks):
                        ...
                        if classifier_select_mode == "compose":
                            logits[:, start:end] = (
                                self.classifiers[t_id](pooled_output)
                                * task_id_prob[:, t_id][:, None]
                            )
        ...
        #     return ((loss,) + output) if loss is not None else output
            return SequenceClassifierOutput(
                loss=loss + match_loss if loss is not None else match_loss,
                logits=logits,
        ...
        if train:
            try:
                self.prefix_encoder.writer.add_scalar(
                    f"task_loss/task_{task_id}",
                    loss.cpu(),
                    self.prefix_encoder.steps[task_id],
                )
                self.prefix_encoder.writer.add_scalar(
                    f"total_loss/task_{task_id}",
                    (loss + match_loss).cpu(),
                    self.prefix_encoder.steps[task_id],
                )
            except:
    ...
    def process_query(self, embed, attention_mask, train, task_id, labels=None):
        if "cls_token_embed" in self.config.query_encoder_type:
            ...
            x_query = torch.sum(
                embed * attention_mask.unsqueeze(-1), dim=1
            ) / torch.sum(attention_mask, dim=1).unsqueeze(-1)

        # store embedding prototypes if required
        if (
            train
            and self.config.classifier_match_embed
            and type(self.embed_prototypes[task_id]) == list
        ):
            self.embed_prototypes[task_id].extend(x_query.tolist())

        if (
            train
            and self.config.task_identify_epi
            and len(self.task_means_over_classes) < task_id + 1
        ):
    ...
    def _get_pred_ids(self, x_query, task_id, final, log_id="cil"):
        if self.config.classifier_match_embed:
            ...
            cos_sim = torch.cosine_similarity(
                x_query.unsqueeze(1), embed_prototypes.unsqueeze(0), dim=-1
            )  # [bs, num_tasks]
            prob = nn.functional.softmax(
                cos_sim * self.config.softmax_match_scale, dim=-1
            )  # if not self.config.direct_compose else cos_sim; comment this because for the prediction, softmax is required and scale is necessary in this case.
            pred_ids = torch.argmax(prob, dim=-1)  # [bs]

        elif self.config.classifier_match_key:
            K = self.prefix_encoder.keys
            cos_sim = torch.cosine_similarity(
                x_query.unsqueeze(1), K.unsqueeze(0), dim=-1
            )  # [bs, num_tasks]
            prob = nn.functional.softmax(
                cos_sim * self.config.softmax_match_scale, dim=-1
            )  # if not self.config.direct_compose else cos_sim
            pred_ids = torch.argmax(prob, dim=-1)  # [bs]
        ...
        self.prefix_encoder.writer.add_scalar(
            f"key_match_acc/{log_id}_task_{task_id}", accuracy.mean(), step
        )
